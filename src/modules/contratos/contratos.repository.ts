...
import { ContratoResumen, ContratoDetalle, CuotaAmortizacion, CuotaAdicional } from './contratos.interface';
...
export class ContratosRepository {
    ...
    private async buscarApoderado(connection: oracledb.Connection, dfacProducto: number): Promise<string> {
        try {
            ...
            `;
            const result: any = await connection.execute(
                sql, [CODIGO_EMPRESA, dfacProducto], { outFormat: oracledb.OUT_FORMAT_OBJECT }
            );
