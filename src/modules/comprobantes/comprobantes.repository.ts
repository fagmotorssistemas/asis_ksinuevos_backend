...
import { ComprobanteImagen } from './comprobantes.interface';

/** Por defecto 162; process.env.CCO_EMPRESA_DEFAULT lo sobrescribe cuando migres a .env. */
const DEFAULT_CCO_EMPRESA = 162;
const defaultEmpresa = () =>
    parseInt(process.env.CCO_EMPRESA_DEFAULT || String(DEFAULT_CCO_EMPRESA), 10);

const assertOracleIdentifier = (name: string, envKey: string): string => {
    const n = name.trim();
    if (!/^[A-Za-z][A-Za-z0-9_$#]*(\.[A-Za-z][A-Za-z0-9_$#]*)?$/.test(n)) {
        throw new Error(
            `Nombre de objeto Oracle inválido en ${envKey}. Use solo letras, números, _, $, # y opcionalmente esquema.objeto.`
        );
    }
    return n;
};

const listComprobaView = () =>
    assertOracleIdentifier(
        process.env.ORACLE_LIST_COMPROBA_VIEW || 'LIST_CCOMPROBA_V',
        'ORACLE_LIST_COMPROBA_VIEW'
    );

const imagenTable = () =>
    assertOracleIdentifier(
        process.env.ORACLE_CCOMPROBA_IMAGEN_TABLE || 'CCOMPROBA_IMAGEN',
        'ORACLE_CCOMPROBA_IMAGEN_TABLE'
    );

const parseOracleDate = (dateVal: unknown): string | undefined => {
...
const mapRowKeysToCamel = (row: Record<string, unknown>): Record<string, unknown> => {
...
export class ComprobantesRepository {
    async listarComprobantesPorEmpresa(empresa: number): Promise<Record<string, unknown>[]> {
        let connection;
