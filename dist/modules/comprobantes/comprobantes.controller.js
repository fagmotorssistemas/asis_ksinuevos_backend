...
const getListadoComprobantes = async (req, res) => {
    try {
...
exports.getListadoComprobantes = getListadoComprobantes;
const getImagenesComprobante = async (req, res) => {
    try {
        const empresa = empresaFromQuery(req);
        if (isNaN(empresa)) {
            res.status(400).json({
...
const postSubirImagenComprobante = async (req, res) => {
    try {
        ...
        const creaUsr = (typeof req.body?.creaUsr === 'string' && req.body.creaUsr.trim()) ||
            (typeof req.headers['x-usuario'] === 'string' && req.headers['x-usuario'].trim()) ||
            'API_USR';
        const resultado = await service.subirImagenYRegistrar(empresa, ccoCodigo, file.buffer, file.mimetype, file.originalname || 'adjunto', creaUsr);
        res.status(201).json({
            success: true,
            data: resultado
        });
...
exports.postSubirImagenComprobante = postSubirImagenComprobante;
