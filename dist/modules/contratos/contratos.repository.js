...
// src/api/contratos/contratos.repository.ts
...
class ContratosRepository {
    ...
    async buscarApoderado(connection, dfacProducto) {
        try {
            ...
            return result.rows.length > 0 ? result.rows[0].DATOS_APODERADO : 'No registrado / Compra Directa';
        }
        catch (error) {
    ...
    async getListaPagosAdicionales(connection, ccoCodigo, codigoPago) {
        try {
            ...
            `;
            const result = await connection.execute(sql, { ccoCodigo: ccoCodigo, empresa: CODIGO_EMPRESA, codigoPago: codigoPago }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
            return result.rows.map((row) => ({
                monto: row.MONTO,
                letras: row.LETRAS,
    ...
    // --- HELPER: Obtener Pago Único (Suma para el total) ---
    async getPagoEspecifco(connection, ccoCodigo, codigoPago) {
        try {
            const sql = `
                SELECT 
            ...
            if (result.rows && result.rows.length > 0) {
                return { monto: result.rows[0].MONTO, letras: result.rows[0].LETRAS };
            }
            return { monto: 0, letras: 'CERO 00/100' };
        }
        catch (error) {
            console.error(`Error en getPagoEspecifco código ${codigoPago}:`, error);
            return { monto: 0, letras: 'Error' };
        }
    }
    // CONSULTA 2: Detalle Completo
        ...
        try {
            ...
            const sqlPrincipal = `
                SELECT 
                    ...
                    CLIENTE, SIS_NOMBRE, CCO_FECHA, CCO_FECHA_DADO, CCO_FECHACR, CCO_FECHA_CI, CCO_FECHA1, 
                ...
                WHERE CCO_CODIGO = :ccoCodigo AND ROWNUM = 1
            `;
            const result = await connection.execute(sqlPrincipal, [ccoCodigo], { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
    ...
    async getAmortizacionPorContrato(ccoCodigo) {
        ...
        try {
            connection = await (0, oracle_1.getConnection)();
            const sql = `
                SELECT ddo_cco_comproba, FEC_VENCE, NCUOTA, CAPITAL, SSO_INTERES, CUOTA, SALDO_CAPITAL
                FROM (
                    SELECT a.ddo_cco_comproba, 
                           ...
                           prg_usr.prueba.valor_amortizacion(a.ddo_empresa,a.ddo_cco_comproba,a.ddo_pago,1) as SALDO_CAPITAL -- Simplificado para el ejemplo
